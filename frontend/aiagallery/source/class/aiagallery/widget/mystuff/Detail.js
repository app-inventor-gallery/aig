          ...
          rpc.callAsync(
            function(e)
            {
              ...
              channelMessage = function(type, data)
              {
                ...
                if (type == "open")
                {
                  qx.util.TimerManager.getInstance().start(
                    function()
                    {
                      ...
                      socket = this.getUserData("channelSocket");
                      if (socket)
                      {
                        socket.close();
                      }
                      this.setUserData("channelSocket", null);
                      socket = null;
                ...
                {
                  this.debug("Channel Message (" + type + ")");
                }
                else
                {
                  this.debug(liberated.dbif.Debug.debugObjectToString(
                                data,
              ...
              alert("Looking for old channel");
              socket = this.getUserData("channelSocket");
              if (socket)
              {
                alert("Close old channel");
                // ... then close it
                socket.close();
              }
              ...
              this.setUserData("channelSocket", socket);
